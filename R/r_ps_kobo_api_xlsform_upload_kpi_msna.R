...
kobo_form_xlsx_folder<-"./xlsform/tur/"
## get list of files
...
for (kobo_form_xlsx in f_list){
  ...
  #add to datafrme
  d_import_url<-bind_rows(d_import_url,as.data.frame(import_url))
}
...
for (i_row in 1:nrow(d_import_url)){
  ...
  #
  d_asset_uid<-bind_rows(d_asset_uid,as.data.frame(asset_uid))
}
...
for (i_row in 1:nrow(d_asset_uid)){
  ...
  for (permission_i in permission_list){
    ...
    print (paste0(d_content$user, "--",permission_i))
  }
}
